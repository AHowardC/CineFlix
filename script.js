...
var searchMoviesUrl =
  ...
  currentPage;
var discoverBaseUrl =
  apiBaseUrl + "/discover/movie?api_key=" + apiKey + "&page=" + currentPage;
var upcomingBaseUrl =
  apiBaseUrl +
...
$(document).ready(function () {
  ...
  $(".fa-search").click(function (event) {
    ...
    if (
      $(".search-field").hasClass("active-search") &&
      $(".search-field").val() !== ""
    ) {
      $(".search-form").submit();
    } else {
  ...
  $(window).scroll(function () {
    ...
    if (
      $(window).scrollTop() >= newCallStart &&
      $(window).scrollTop() <= newCallEnd
    ) {
      currentPage += 1;
      nowPlayingUrl =
        apiBaseUrl +
  ...
  function searchMovies(searchQuery) {
    ...
    $.getJSON(searchMoviesUrl, function (searchData) {
      ...
      for (let i = 0; i < searchData.results.length; i++) {
        ...
        searchHTML +=
          '<div class="movie-item col-sm-6 col-md-4 col-lg-3" id="' +
          searchID +
          '" data-toggle="modal" data-target=".movie-modal" onclick="updateModal(this);">';
        ...
        searchHTML +=
          "<h4>Release Date: " + month + " " + day + ", " + year + "<h4>";
        searchHTML += "</div>";
        ...
        for (let j = 0; j < savedArray.length; j++) {
          if (savedArray.indexOf(searchID.toString()) === -1) {
            searchHTML +=
              '<p><span id="overlay-heart"><i class="fa fa-3x fa-heart-o" aria-hidden="true"></i></span></p>';
            break;
          } else {
            searchHTML +=
              '<p><span id="overlay-heart"><i class="fa fa-3x fa-heart" aria-hidden="true"></i></span></p>';
            break;
          }
        }
        ...
        searchHTML +=
          '<div class="star-ratings"><a href="#" data-toggle="tooltip" data-placement="top" title="' +
          ratingAvg * 10 +
          '%">';
        searchHTML +=
          '<div class="star-ratings-top" style="width: ' +
          ratingAvg * 10 +
  ...
  function discoverJSON(linkVar, sortVar) {
    ...
    $.getJSON(discoverUrl, function (discoverData) {
      ...
      for (let i = 0; i < discoverData.results.length; i++) {
        ...
        var year = protoDate.getFullYear(release);
        var poster =
          imageBaseUrl + "/w300" + discoverData.results[i].poster_path;
        if (poster === "http://image.tmdb.org/t/p/w300null") {
          poster = placeholderImage;
        }
  ...
  $.getJSON(currentUrl, function (detailsData) {
    ...
    var backdropRotate = setInterval(function () {
      ...
      backdrop =
        imageBaseUrl +
        "/w600" +
        detailsData.images.backdrops[backdropCounter].file_path;
    ...
    if (webSite != "") {
      infoHTML += "<p>";
      infoHTML +=
        '<span id="trailer-btn" class="btn btn-secondary"><a href="' +
        webSite +
        '" target="_blank">Visit the Website</a></span>';
      infoHTML +=
        '<span id="trailer-btn" class="btn btn-secondary"><a id="trailer-link" href="#" data-toggle="modal" data-target=".trailer-modal"  data-theVideo="https://www.youtube.com/embed/' +
        trailerId +
        '">View the trailer</a></span>';
      if (protoDate.getTime() > Date.now() - 3888000000) {
        ticketsHTML += '<div class="text-center">';
        ticketsHTML +=
          '<a href="http://www.fandango.com/moviesintheaters" target="_blank" id="tickets-button" class="btn btn-lg btn-primary"> Get Tickets</a>';
        ticketsHTML += "</div>";
      }
      infoHTML += "</p>";
    }
    ...
    ratingHTML +=
      'Rating: &nbsp; <div class="star-ratings"><a href="#" data-toggle="tooltip" data-placement="top" title="' +
      ratingAvg * 10 +
      '%">';
    ratingHTML +=
      '<div class="star-ratings-top" style="width: ' +
    ...
    ratingHTML +=
      " &nbsp; &nbsp; (" +
      ratingCount +
      ' reviews) &nbsp; | &nbsp; Favorite: <i id="heart" class="fa fa-heart-o" aria-hidden="true"></i>';
    ratingHTML += "</div>";
    ratingHTML += "</div>";

    // This added a new sub-modal on top of the main modal for the trailer video player.
    trailerHTML +=
      '<iframe id="player" src="" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>';
    ...
    $("#heart").click(function () {
      ...
      function updateFavorites(favorite) {
        var favoritesUrl =
          apiBaseUrl + "/movie/" + favorite + "?api_key=" + apiKey;
        $.getJSON(favoritesUrl, function (favoritesData) {
          var title = favoritesData.original_title;
          var release = favoritesData.release_date;
          var protoDate = new Date(release);
